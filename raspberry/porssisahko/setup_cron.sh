...
(
    ...
    echo "21 15,22 * * * cd $SCRIPT_DIR && venv/bin/python3 porssisahko_read.py >> $SCRIPT_DIR/logs/cron_read.log 2>&1"
    # Charging optimization: runs at 4 AM and 4 PM daily (after price fetch)
    echo "22 15,22 * * * cd $SCRIPT_DIR && venv/bin/python3 porssisahko_optimize_charging.py >> $SCRIPT_DIR/logs/cron_opt.log 2>&1"
